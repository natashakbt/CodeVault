...
### NEWER VERSION IN AUTOSAVE CACHE??????
...
files = glob.glob(os.path.join(single_clust_dir, '*'))
for file in files:
    os.remove(file)
...
# %% BEFORE/AFTER COUNTS ON A PER-SESSION BASIS
# -------------------------------
# Load and prepare summary data
# -------------------------------
summary_df = pd.DataFrame(summary_data)
summary_df = summary_df[summary_df['cluster_num'] != -2.0]  # Remove no movement cluster
summary_df.to_pickle("/home/natasha/Desktop/clustering_data/summary_df.pkl")  

# Unique combinations of session, cluster, and taste
unique_combinations = summary_df[['cluster_num', 'taste_name', 'basename']].drop_duplicates()

# -------------------------------
# Count movements per trial
# -------------------------------
count_df = (
    summary_df.groupby(['basename', 'cluster_num', 'event_position', 'taste_name', 'trial'])
    .size()
...
# -------------------------------
# Run statistics per session/trial
# -------------------------------
session_trial_results = []

for _, combo in unique_combinations.iterrows():
    basename = combo['basename']
    cluster = combo['cluster_num']
    taste = combo['taste_name']
    ...
    num_trials = len(matching_rows['trial'].unique())
    
    # Handle sessions with no data
    if num_trials == 0:
    ...
    # Prepare trial-by-trial DataFrame
    ttest_df = pd.DataFrame(0, index=range(num_trials), columns=['trial', 'before', 'after'])
    ttest_df['trial'] = matching_rows['trial'].unique()

    for _, row in matching_rows.iterrows():
        trial = row['trial']
        event_pos = row['event_position']
        ttest_df.loc[ttest_df['trial'] == trial, event_pos] = row['movement_count']

    # Run paired t-test and Poisson means test
    before_counts = ttest_df['before'].tolist()
    after_counts = ttest_df['after'].tolist()

   # t_stat, t_pval = ttest_rel(before_counts, after_counts)
    res = stats.poisson_means_test(sum(before_counts), len(before_counts),
                             sum(after_counts), len(after_counts))

    sig_status = 'yes' if res.pvalue < 0.05 else 'no'
...
# -------------------------------
print("\nPOISSON MEANS TEST: significant results")
sig_results = session_trial_results_df[session_trial_results_df['poisson_pvalue'] < 0.05]
...
for basename in tqdm(unique_basenames):
    ...
    for (row_val, col_val), ax in g.axes_dict.items():
        ...
        if len(res_row) == 0:
            ...
            n_trials = subsub['trial'].nunique() if 'trial' in subsub.columns else len(subsub)
            label = f"p = {pval:.2g}\nn = {n_trials}"
...
# -------------------------------
# Generate uniform distribution with randomized data
# -------------------------------
# Parameters
n_values = len(session_trial_results_df)
n_reps = 50
binwidth = 0.05
...
random_data = np.random.uniform(0, 1, size=(n_reps, n_values)) # Generate random uniform(0,1) data
counts_per_rep = np.array([np.histogram(rep, bins=bins)[0] for rep in random_data]) # Histogram counts for each repetition
mean_counts = counts_per_rep.mean(axis=0) # Mean across reps
se_counts = counts_per_rep.std(axis=0, ddof=1) / np.sqrt(n_reps)# SE across reps
...
first_bin_count = counts[0] # real count of how many significant test there are
...
'''
'''
# %% # BEFORE/AFTER TRANSITION COUNTS WITH STATS TESTS - EVERY DATA POINT IS A SESSION - ARCHIVE
