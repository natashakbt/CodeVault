...
# ==============================================================================
# Set window length and transition time to look at
# I suggest window_len = 500 and fixed_transition_time = 2800
# ==============================================================================
window_len = 500 # Half of the total window to evaluate around transition window (in ms)
fixed_transition_time = 2800# Set to a fixed time from stimulus delivery (2000ms is stim delivery).


# ==============================================================================
# Setting up folders to save plots
# ==============================================================================
fig_dir = os.path.join(dirname, 'mtm_event_pos_divergence')
...
if fixed_transition_time >= 2000:
    print(f"Using fixed transition time of {fixed_transition_time}ms")
    expanded_df['scaled_mode_tau'] = fixed_transition_time
else:
    print("Pick a transition time after stimulus delivery (2000ms+)")
...
for idx, row in transition_events_df.iterrows():
    ...
    event_position = "before" if segment_bounds[1] < fixed_transition_time else "after"
...
for n in tqdm(range(n_iterations)):
    for basename in unique_sessions:
        ...
        divergence_p = 1 - (stats.percentileofscore(sh_stat, actual_stat)/100)
...
save_file_path = os.path.join(dirname, 'UMAP_of_all_MTMs_plot_df.pkl')

# Save as pickle
plot_df.to_pickle(save_file_path)

print(f"Saved plot_df as a pickle file to {save_file_path}")





# %% TRYING PCA  -- DOESN'T WORK RIGHT NOW
